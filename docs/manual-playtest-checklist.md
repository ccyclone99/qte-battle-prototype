...
- Difficulty, weapon, and enemy matchup controls are readable.
...
- Auto encounter opens `反制入门`, not the full pressure dojo.
- The beginner route layers single clash, two-hit read, guard/contact frame, spell interrupt, and follow-up conversion before mixed pressure.
- Selecting Greatsword, Staff, Dual Blades, or Sword-Shield changes weapon identity and QTE chains without exposing old style IDs.
- A compact learning objective appears during active combat and changes when the state changes.
- Enemy physical chains ask for sequential node responses instead of one input covering the whole chain.
- `逆势试炼` eventually shows quick triple pressure, delayed cleave pressure, spell-to-melee pressure, and shield-crush pressure.
...
- Follow-up learning objective says A/S/D is now allowed and no input becomes unbonused automatic attack.
...
- Fire encounter opening heat remains available through the first enemy action, then cools on later enemy-turn boundaries.
- Burn reads as enemy turn-start DOT, not as a hidden next-fire-hit amplifier.
- Whiffed QTE attacks do not grant positive heat/status rewards, while explicit spender costs still feel committed at cast time.
- Win/loss screen shows encounter, phase reached, accuracy, Perfect count, max combo, damage, and hits taken.
- Win/loss screen can copy local telemetry JSON and reports success/failure of clipboard access.

## Visual Readability

- Greatsword, Dual Blades, Staff, shield, and casting poses are distinguishable without reading text.
- The learning objective panel does not cover actor bodies, weapon contact, QTE timing bar, or result screen summary.
...
- `docs/qte-prototype-reference.md` includes a formal transfer contract for input, QTE chains, enemy attacks/chains, active attacks, hit confirm, learning objective, telemetry, and renderer boundaries.
...
- QTE debug shows guard stance state, learning objective, weapon identity, active animation events, local telemetry export hint, and damage path audit lines.
- `window.exportCombatTelemetry()` includes `weaponIdentity`, `learningObjective`, `feedback`, `difficultyAssist`, `enemyDirector`, `resourcePolicy`, and `animationEvents`.
